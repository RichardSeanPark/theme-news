...
from theme_news_agent.sub_agents.data_collection.tools.search_trend_tool import SearchTrendTool # 클래스 임포트 추가

# --- Fixtures ---

@pytest.fixture
def search_trend_tool_instance():
    """Provides an instance of SearchTrendTool for tests."""
    return SearchTrendTool()

# --- 테스트 케이스 ---

# 2.5.1: fetch_google_trends 성공 테스트 (Live API)
def test_fetch_google_trends_success(search_trend_tool_instance, caplog): # fixture 추가
    ...
    try:
        # 한국 지역 테스트
        # 인스턴스를 통해 .func 호출
        trends_kr = search_trend_tool_instance.fetch_google_trends.func(region='KR')
        ...
        # trends_us = search_trend_tool_instance.fetch_google_trends.func(region='US') # 인스턴스 사용
...
def test_fetch_google_trends_pytrends_error(mock_trending_searches, search_trend_tool_instance, caplog): # fixture 추가
    ...
    # 인스턴스를 통해 .func 호출
    trends = search_trend_tool_instance.fetch_google_trends.func(region='KR')
...
def test_fetch_naver_datalab_trends_placeholder(search_trend_tool_instance, caplog): # fixture 추가
    """fetch_naver_datalab_trends 호출 시 빈 리스트 및 경고 로그 확인"""
    caplog.set_level(logging.WARNING)

    # 인스턴스를 통해 .func 호출
    trends = search_trend_tool_instance.fetch_naver_datalab_trends.func()
