...
def fetch_google_trends_func(region: str = 'KR') -> list[dict]:
    """
    ...
    try:
        ...
        logger.error(f"Error fetching Google Trends for region {region}: {e}")

    return trending_searches_data

def fetch_naver_datalab_trends_func() -> list[dict]:
    """
...
fetch_google_trends = FunctionTool(func=fetch_google_trends_func)
fetch_naver_datalab_trends = FunctionTool(func=fetch_naver_datalab_trends_func)
