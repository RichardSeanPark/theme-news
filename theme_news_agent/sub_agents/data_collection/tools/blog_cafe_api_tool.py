...
def _parse_naver_datetime(datetime_str: str) -> str | None:
    """Naver API 날짜 형식(RFC 1123)을 ISO 8601 형식으로 변환"""
    if not datetime_str:
        return None
    try:
...
def _fetch_naver_search_api(api_type: str, query: str, display: int = 100, sort: str = 'date') -> list[dict]:
    ...
    try:
        ...
        for item in items:
            ...
            # 카페는 'cafeurl' + 'cafename' 조합을 source로, 블로그는 'bloggername'을 source로 사용
            source_name = f"{item.get('cafename')} ({item.get('cafeurl')})" if api_type == 'cafearticle' else item.get('bloggername', 'Naver Blog')

            articles_data.append({
                "title": title,
                "content": description, # description 필드를 content로 사용
                "source": source_name,
                "published": _parse_naver_datetime(item.get("postdate" if api_type == 'blog' else "pubDate")), # 블로그는 postdate, 카페는 pubDate 사용 시도 (API 문서 확인 필요 - 문서에는 pubDate)
                "url": item.get("link")
            })
...
def fetch_naver_blogs_func(query: str, display: int = 100, sort: str = 'date') -> list[dict]:
    """
...
def fetch_naver_cafe_articles_func(query: str, display: int = 100, sort: str = 'date') -> list[dict]:
    """
    ...
    # 그러나 API 호출 시 date 사용해도 오류는 안나므로 일단 유지
